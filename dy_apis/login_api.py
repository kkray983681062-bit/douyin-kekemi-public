...
class DYLoginApi:
    ...
    def dyGenerateQRcode(self, auth) -> dict:
        ...
        params.with_a_bogus()
        resp = requests.get(self.base_url + api, headers=headers.get(), cookies=auth.cookie, params=params.get(), verify=True)
    ...
    def dyGeneratePhoneVerificationCode(self, phone_num, auth):
        ...
        response = requests.post(self.base_url + api, headers=headers, cookies=auth.cookie, params=params.get(), data=data, verify=True)
    ...
    def persistenceLoginInfo(self, auth):
        ...
        params.add_param("msToken", auth.cookie['msToken'])
        params.with_a_bogus()
        response = requests.get(url, headers=headers, cookies=auth.cookie, params=params.get(), verify=True)
    ...
    async def phoneMain(self):
        ...
        response = requests.get(redirect_url, headers=headers, cookies=auth.cookie, verify=True)
        print(response.status_code)
        if response.status_code == 302:
            print('Redirect received; response headers are hidden.')
            location = response.headers['Location']
            response = requests.get(location, headers=headers, cookies=auth.cookie, verify=True)
            auth.cookie.update(response.cookies.get_dict())
            if response.status_code == 302:
                print('Redirect received; response headers are hidden.')
                location = response.headers['Location']
        ...
        print('Login complete; credential values are hidden.')
