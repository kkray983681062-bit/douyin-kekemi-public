...
async def get_ttwid_and_webid():
    url = os.environ.get('DY_PROFILE_URL', '').strip()
    if not url:
        raise ValueError('Configure DY_PROFILE_URL with an authorized profile URL')
    headless = False
    print('如出现验证过程，请手动验证')
...
if __name__ == '__main__':
    result = get_new_cookies()
    print(f"Cookie fields collected: {len(result['cookies'])}; values are hidden.")
