...
def generate_ttwid():
    url = f"https://www.douyin.com/discover"
    ttwid = None
    try:
        headers = {
...
def generate_webid(auth=None, url=""):
    ...
    try:
        ...
        response = requests.get(url, headers=headers.get(), verify=True)
...
def generate_csrf_token(cookies_str):
    ...
    try:
        ...
        response = requests.head('https://www.douyin.com/service/2/abtest_config/', headers=headers, verify=True)
